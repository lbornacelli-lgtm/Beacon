...
def _synthesise_elevenlabs(text: str, path: str):
    """
    Synthesise using ElevenLabs API and save as MP3.
    Path will have .mp3 extension substituted automatically.
    The audio_chain.py handles MP3 → WAV conversion at broadcast time.
    """
    from elevenlabs.client import ElevenLabs
    from elevenlabs import VoiceSettings

    # Save as MP3 — audio_chain handles conversion at broadcast time
    ...
    # Return mp3_path by renaming path variable — caller uses wav_path
    # but we store the mp3 path in MongoDB so audio_chain knows to convert
    if mp3_path != path:
        os.replace(mp3_path, path.replace(".wav", ".mp3"))
...
def _synthesise(text: str, path: str, voice=None):
    """Synthesise text to audio using the best available TTS engine.
    ElevenLabs saves MP3, Piper saves WAV. audio_chain handles both.
    """
    if ELEVENLABS_API_KEY:
        # Use .mp3 extension for ElevenLabs output
